...
//实现日志创建、写入方式的判断;通过单例模式获取唯一的日志类，调用init方法，初始化生成日志文件，
...
{   //异步需要设置阻塞队列的长度，同步不需要设置
    ...
    //获取当前的日历时间。若 timer 为NULL，则返回日历时间；若timer非NULL则将获取的日历时间存于timer中
    time_t t = time(NULL);
    //struct tm*  localtime (const  time_t*  timer)；
    //将time_t类型的日历时间转换为 tm 结构的 Local time（本地时间）
    ...
    else    //将/的位置向后移动一个位置，然后复制到logname中
...
void Log::write_log(int level, const char *format, ...)
{
    ...
    //日志不是今天或写入的日志行数是最大行的倍数(m_split_lines为最大行数),则新建log文件
    ...
    //int snprintf(char *str, size_t size, const char *format, ...) 设将可变参数(...)按照 format 
    //格式化成字符串，并将字符串复制到 str 中，size 为要写入的字符的最大数目，超过 size 会被截断
    ...
    if (m_is_async && !m_log_queue->full()) //若m_is_async为true表示异步，默认为同步
